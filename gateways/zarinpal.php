	...
	class EDD_ZarinPal_Gateway {
		...
		public function process( $purchase_data ) {
			...
			if ( $payment ) {
				...
				///////////////////////////////////////////////////////////////////////
				$orderId= $payment.'|'.$purchase_data['user_info']['first_name'].' '.$purchase_data['user_info']['last_name'];
				if(edd_get_currency() == 'IRT' ){
					//	$amount = $amount * 10;

					if($email==""){
						$data = array("merchant_id" => $merchant,
							"amount" => $amount,
				...
				if ($err) {
					...
					if (empty($result['errors'])) {
						if ($result['data']['code'] == 100) {
							...
							edd_insert_payment_note( $payment, 'شناسه سفارش: ' . $orderId );
		...
		public function verify() {
			...
			if ( isset( $_GET['Authority'] ) ) {
				...
				$data = array("merchant_id" => $merchant, "authority" => $authority, "amount" => $amount);
				$jsonData = json_encode($data);
